...
var i18n = {input:{}};
i18n.input.offline = {};
i18n.input.offline.latin = {};
i18n.input.offline.latin.DataLoader = function $i18n$input$offline$latin$DataLoader$(inputTool) {
  this.currentInputTool = inputTool;
  this.ready = !1;
  this.scripts_ = [];
  this.dataURL = "data/";
};
goog.inherits(i18n.input.offline.latin.DataLoader, goog.Disposable);
i18n.input.offline.latin.DataLoader.prototype.disposeInternal = function $i18n$input$offline$latin$DataLoader$$disposeInternal$() {
  delete this.dicWords;
  delete this.dicScores;
  this.ready = !1;
  goog.array.forEach(this.scripts_, function(script) {
    goog.dom.removeNode(script);
  });
  this.scripts_ = [];
};
i18n.input.offline.latin.DataLoader.prototype.load = function $i18n$input$offline$latin$DataLoader$$load$(callBackFn) {
  if (this.ready) {
    callBackFn();
  } else {
    var script = document.createElement("script");
    script.src = this.dataURL + this.currentInputTool + ".js";
    document.body.appendChild(script);
    var self = this;
    script.onload = script.onreadystatechange = function $script$onreadystatechange$() {
      this.readyState && "loaded" != this.readyState && "complete" != this.readyState || (self.dicWords = window.dicWords, self.dicScores = window.dicScores, self.ready = !(!self.dicWords || !self.dicScores), callBackFn());
    };
    this.scripts_.push(script);
  }
};
i18n.input.offline.latin.Preprocessor = function $i18n$input$offline$latin$Preprocessor$() {
  var accentChSet = "\u00e0\u00c0\u00e1\u00c1\u00e2\u00c2\u00e3\u00c3\u00e4\u00c4\u00e6\u00c6\u0105\u0104\u00e7\u00c7\u0107\u0106\u0119\u0118\u00e8\u00c8\u00e9\u00c9\u00ea\u00ca\u00eb\u00cb\u011f\u011e\u00ec\u00cc\u00ed\u00cd\u00ee\u00ce\u00ef\u00cf\u0131\u0130\u0142\u0141\u00f1\u00d1\u0144\u0143\u00f2\u00d2\u00f3\u00d3\u00f4\u00d4\u00f5\u00d5\u00f6\u00d6\u0153\u0152\u015b\u015a\u015f\u015e\u00f9\u00d9\u00fa\u00da\u00fb\u00db\u00fc\u00dc\u00ff\u0178\u017a\u0179\u017c\u017b\u0161\u0160\u00e5\u00c5\u00f8\u00d8\u017e\u017d".split("");
  this.diacriticMap_ = {};
  for (var i = 0;i < accentChSet.length;++i) {
    this.diacriticMap_[accentChSet[i]] = "aAaAaAaAaAaAaAcCcCeEeEeEeEeEgGiIiIiIiIiIlLnNnNoOoOoOoOoOoOsSsSuUuUuUuUyYzZzZsSaAoOzZ"[i];
  }
};
i18n.input.offline.latin.Preprocessor.prototype.unaccent = function $i18n$input$offline$latin$Preprocessor$$unaccent$(accentStr) {
  var self = this;
  return accentStr.replace(/[^\u0000-\u007E]/g, function(ch) {
    return self.diacriticMap_[ch] || ch;
  });
};
i18n.input.offline.latin.Candidate = function $i18n$input$offline$latin$Candidate$(word, score) {
  this.word = word;
  this.score = Number(score);
};
i18n.input.offline.latin.Decoder = function $i18n$input$offline$latin$Decoder$(inputTool, opt_callback) {
  this.dataLoader_ = new i18n.input.offline.latin.DataLoader(inputTool);
  this.readyCallback_ = opt_callback || null;
  this.preprocessor_ = new i18n.input.offline.latin.Preprocessor;
  this.dataLoader_.load(this.readyCallback_);
};
goog.inherits(i18n.input.offline.latin.Decoder, goog.Disposable);
i18n.input.offline.latin.Decoder.prototype.isReady = function $i18n$input$offline$latin$Decoder$$isReady$() {
  return this.dataLoader_.ready;
};
i18n.input.offline.latin.Decoder.prototype.decode = function $i18n$input$offline$latin$Decoder$$decode$(source, num) {
  var candidates = this.getCandidates_(source);
  goog.array.sort(candidates, function(candidate1, candidate2) {
    return candidate2.score - candidate1.score;
  });
  var results = [];
  goog.array.forEach(candidates, function(candidate) {
    if (results.length < num && !goog.array.contains(results, candidate.word)) {
      var result = this.canonicalize_(source, candidate.word);
      results.push(result);
    }
  }, this);
  return results;
};
i18n.input.offline.latin.Decoder.prototype.getCandidates_ = function $i18n$input$offline$latin$Decoder$$getCandidates_$(prefix) {
  var source = this.preprocessor_.unaccent(prefix), words = this.dataLoader_.dicWords, scores = this.dataLoader_.dicScores, index = goog.array.binarySearch(words, source, goog.bind(this.compare_, this));
  0 > index && (index = -1 - index);
  for (var startIndex = index;0 <= startIndex && 0 == this.preprocessor_.unaccent(words[startIndex]).indexOf(source);) {
    startIndex--;
  }
  startIndex++;
  for (var endIndex = index;endIndex < words.length && 0 == this.preprocessor_.unaccent(words[endIndex]).indexOf(source);) {
    endIndex++;
  }
  for (var results = [], i = startIndex;i < endIndex;i++) {
    0 == words[i].indexOf(prefix) && results.push(new i18n.input.offline.latin.Candidate(words[i], scores[i]));
  }
  return results;
};
i18n.input.offline.latin.Decoder.prototype.canonicalize_ = function $i18n$input$offline$latin$Decoder$$canonicalize_$(source, target) {
  if (source == source.toUpperCase()) {
    return target.toUpperCase();
  }
  var initChar = source.charAt(0), otherChars = source.slice(1);
  return initChar == initChar.toUpperCase() && otherChars == otherChars.toLowerCase() ? target.charAt(0).toUpperCase() + target.slice(1) : target;
};
i18n.input.offline.latin.Decoder.prototype.compare_ = function $i18n$input$offline$latin$Decoder$$compare_$(first, second) {
  var unaccent_first = this.preprocessor_.unaccent(first), unaccent_second = this.preprocessor_.unaccent(second);
  return unaccent_first < unaccent_second ? -1 : unaccent_first == unaccent_second ? 0 : 1;
};
i18n.input.offline.latin.Decoder.prototype.disposeInternal = function $i18n$input$offline$latin$Decoder$$disposeInternal$() {
...
i18n.input.chrome.DataSource.prototype.setCorrectionLevel = function $i18n$input$chrome$DataSource$$setCorrectionLevel$() {
};
i18n.input.chrome.DataSource.EventType = {CANDIDATES_BACK:"cb$0", READY:"r$1"};
i18n.input.chrome.DataSource.CandidatesBackEvent = function $i18n$input$chrome$DataSource$CandidatesBackEvent$(source) {
...
i18n.input.chrome.xkb.JsDataSource = function $i18n$input$chrome$xkb$JsDataSource$(numOfCandidate, callback) {
  i18n.input.chrome.DataSource.call(this, numOfCandidate, callback);
};
goog.inherits(i18n.input.chrome.xkb.JsDataSource, i18n.input.chrome.DataSource);
i18n.input.chrome.xkb.JsDataSource.prototype.setLanguage = function $i18n$input$chrome$xkb$JsDataSource$$setLanguage$(language) {
  i18n.input.chrome.xkb.JsDataSource.superClass_.setLanguage.call(this, language);
  this.decoder_ = new i18n.input.offline.latin.Decoder(language + "-t-i0-und", this.onLoaded_.bind(this));
};
i18n.input.chrome.xkb.JsDataSource.prototype.onLoaded_ = function $i18n$input$chrome$xkb$JsDataSource$$onLoaded_$() {
  this.ready = !0;
};
i18n.input.chrome.xkb.JsDataSource.prototype.sendCompletionRequest = function $i18n$input$chrome$xkb$JsDataSource$$sendCompletionRequest$(query) {
  var candidates = this.decoder_.decode(query, this.numOfCandidate);
  this.callback(query, candidates || [], []);
};
i18n.input.chrome.xkb.JsDataSource.prototype.sendPredictionRequest = function $i18n$input$chrome$xkb$JsDataSource$$sendPredictionRequest$() {
  this.callback("", [], []);
};
goog.exportSymbol("xkb.DataSource", i18n.input.chrome.xkb.JsDataSource);
var dataSourceProto$$inline_4 = i18n.input.chrome.xkb.JsDataSource.prototype;
goog.exportProperty(dataSourceProto$$inline_4, "setLanguage", dataSourceProto$$inline_4.setLanguage);
goog.exportProperty(dataSourceProto$$inline_4, "sendCompletionRequest", dataSourceProto$$inline_4.sendCompletionRequest);
goog.exportProperty(dataSourceProto$$inline_4, "sendPredictionRequest", dataSourceProto$$inline_4.sendPredictionRequest);
goog.exportProperty(dataSourceProto$$inline_4, "seCorrectionLevel", dataSourceProto$$inline_4.setCorrectionLevel);
goog.exportProperty(dataSourceProto$$inline_4, "isReady", dataSourceProto$$inline_4.isReady);
})();
